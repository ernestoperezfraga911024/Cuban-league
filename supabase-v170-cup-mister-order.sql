-- V170: preserve the Mister matchday position through draft, publication, undo and backup restore.
-- No matchday is published or modified by this migration.
alter table public.matchday_stats add column if not exists mister_rank integer;
alter table public.matchday_stats add constraint matchday_stats_mister_rank_range check (mister_rank between 1 and 20);
comment on column public.matchday_stats.mister_rank is 'Position in the Mister matchday table; larger loses a Copa points tie. NULL means unknown.';
alter table public.matchday_drafts add column if not exists mister_rank integer;
alter table public.matchday_drafts add constraint matchday_drafts_mister_rank_range check (mister_rank between 1 and 20);
comment on column public.matchday_drafts.mister_rank is 'Position in the Mister matchday table; larger loses a Copa points tie. NULL means unknown.';
...
begin
  ...
  select jsonb_agg(
    submitted.row_object || jsonb_build_object(
      'mister_rank', case when submitted.row_object ? 'mister_rank'
        then (submitted.row_object ->> 'mister_rank')::integer
        else coalesce(drafts.mister_rank, stats.mister_rank) end,
...
begin
  ...
  insert into public.matchday_stats (
    id, season, matchday, participant_name, mister_rank, points, goals, clean_sheets,
